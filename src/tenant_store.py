...
class TenantStore:
    ...
    def set_cadence(self, schedule_id: int, every_hours: int) -> None:
        """Юзер крутит периодичность в боте — меняется строка, а не воркфлоу n8n."""
        with self.conn.cursor() as cur:
            cur.execute(
                """
                UPDATE schedules
    ...
    def claim_jobs(self, worker_id: str, limit: int = 1) -> list[ClaimedJob]:
        ...
        SKIP LOCKED — то, ради чего это написано: у n8n два Worker'а, и без
        него оба взяли бы одну задачу и отрендерили один шортс дважды.
        """
        with self.conn.cursor() as cur:
            cur.execute(
                """
